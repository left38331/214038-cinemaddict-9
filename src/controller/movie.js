...
export default class MovieController {
  ...
  _removeComment() {
    const allDellBtnComments = this._cardPopup.getElement().querySelectorAll(`.film-details__comment-delete`);
    const entry = this._data;

    allDellBtnComments.forEach((btn, i) => {
      btn.addEventListener(`click`, (e) => {
        e.preventDefault();
        e.target.closest(`.film-details__comment`).remove();
        this._cardPopup.getElement().querySelector(`.film-details__comments-count`).innerHTML = this._cardPopup.getElement().querySelectorAll(`.film-details__comment`).length;
